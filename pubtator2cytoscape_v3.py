...
with open(file_name,'r') as pubtator_pre:
    for line in pubtator_pre:
        if line == '\n':
            ...
            if re.search('\|t\|', line.split('\t')[0]):
                pubmed_id_set.add(line.split('\|')[0])
            elif re.search('\|a\|', line.split('\t')[0]):
                next
            else:
                line = line.strip('\n')
                if(line.split('\t')[5] == ''):
        ...
        if line == '\n':
            ...
            for i in range(0, len(uni_list), 1):
                ...
                for j in range(i+1, len(uni_list), 1):
                    ...
                    if(uni_list[i] == uni_list[j]):
                        ...
                        pair_key = str(node_1)+'\t'+str(name_id_1)+'%'+str(node_2)+'\t'+str(name_id_2)
            ...
            if re.search('\|t\|', line.split('\t')[0]):
                ...
                if line.split('\t')[5] in id_dict.keys():
                    ...
                    pubmed_set.add(line.split('\t')[0]+'#'+node_name+'#'+line.split('\t')[5])
                elif line.split('\t')[4] + '$' + line.split('\t')[3].lower() in id_dict.values():
                    pubmed_set.add(line.split('\t')[0]+'#'+line.split('\t')[4] + '$' + line.split('\t')[3].lower()+'#'+str(get_key(new_id_dict, line.split('\t')[4] + '$' + line.split('\t')[3].lower())))
                else:
                    pubmed_set.add(line.split('\t')[0]+'#'+line.split('\t')[4] + '$' + line.split('\t')[3].lower()+'#'+line.split('\t')[5])

#print("Edge count: " + str(len(class_dict)))
...
with open(str(file_name) + '_edge.txt', 'w') as pubtator_e:
    for name in class_dict.keys():
        ...
        pubtator_e.write(str(name)+'\t'+str(class_dict[name])+'\t'+str(scale_weight)+'\n')
        
#print("Node count: " + str(len(node_set)))
...
#print('Source node count:' + str(len(node_dict)))
...
with open(file_name + '.xgmml', 'w') as pubtator_xgmml:
